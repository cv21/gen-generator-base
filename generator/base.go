...
type (
	generatorParams struct {
		...
		// Also it is useful for building config example.
		// It is better to use module queries if your generator supports versioning.
		// Example: github.com/cv21/gen-generator-mock@v1.0.0
...
`, moduleRepository, moduleQuery, repositoryPath(p.Repository), p.Repository))
}
...
| --- | --- | --- | --- |
`, strings.Join(removeStringsPrefix(GetStringsWithoutPrefix(parsedGeneratorMethod.Docs, prefixComment), prefixExample), lineBreak))
...
`, p.Repository, buildYamlConfigExample(parsedGeneratorParams))
...
func buildYamlConfigExample(s *types.Struct) (r string) {
...
func repositoryPath(repoWithQuery string) string {
	return strings.Split(repoWithQuery, "@")[0]
}
